...
class BaseTrainer(ABC):
    ...
    def __next__(self) -> EpochStats:
        ...
        self.epoch += 1
        self.iter_num += 1
        self.policy.actor.preprocess.epoch = self.epoch
        # self.policy.actor.preprocess.last_test_rew = self.best_reward
        # print(f"Epoch {self.epoch} best reward {self.best_reward}")
        ...
        if not self.stop_fn_flag:
            ...
            if self.test_collector is not None:
                ...
                self.policy.actor.preprocess.last_test_rew = test_stat.returns_stat.mean
